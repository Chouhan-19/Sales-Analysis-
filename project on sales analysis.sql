...
#14. find the total cumulative 
select  sales_id,sales_amount,sum(sales_amount) over(order by sales_id ) as cumulative from salestable;

#Replace the last sales_aount valu with xx
select sales_amount, concat(left(sales_amount,length(sales_amount)-1),"XX") from salestable;

# second heigh sale_amount
select max(sales_amount) from salestable where sales_amount !=(select max(sales_amount) from salestable);

# 14 3 height sales_amount
select s.sales_amount from salestable as s
where 3=
(select count(distinct(s1.sales_amount)) as  s1
where s.sales_amount>s1.sales_amount);
